...
describe("Node Package Unit Tests", () => {
  beforeEach(() => {
    NodeRouter.clearRegistry();
    NodeMarketplace.clearHistory();
  });
  ...
  describe("NodeIdentity", () => {
    it("should return a valid default manifest", () => {
      ...
      expect(manifest.name).toBe("MonadForge Node");
    ...
    it("should throw error if skill package is not found", () => {
      expect(() => NodeIdentity.getSkillPackage("non_existent")).toThrow(
  ...
  describe("NodeRouter", () => {
    it("should register an node and list it via getRegisteredAgents", () => {
      const mockManifest = { agentId: "test-node", pricing: {} };
      NodeRouter.registerAgent("test-node", mockManifest);
      const nodes = NodeRouter.getRegisteredAgents();
      expect(nodes["test-node"]).toBe(mockManifest);
      expect(nodes["monadforge-node"]).toBeDefined();
    });

    it("should clear the registry", () => {
      NodeRouter.registerAgent("test-node", { agentId: "test-node" });
      NodeRouter.clearRegistry();
      const nodes = NodeRouter.getRegisteredAgents();
      expect(nodes["test-node"]).toBeUndefined();
      expect(nodes["monadforge-node"]).toBeDefined();
    });

    it("should invoke local node monadforge-node or monadforge", async () => {
      const res1 = await NodeRouter.invokeAgent(
        "monadforge-node",
        "search_docs",
        { query: "monad" },
      );
      const res2 = await NodeRouter.invokeAgent("monadforge", "search_docs", {
        query: "monad",
      });
    ...
    it("should reject invocation for unknown remote node", async () => {
      await expect(
        NodeRouter.invokeAgent("unknown-node", "search_docs", {
          query: "test",
        }),
      ).rejects.toThrow("Target node 'unknown-node' not found in registry.");
    });

    it("should invoke remote registered node via simulation", async () => {
      const mockManifest = {
        agentId: "remote-node",
        pricing: { search_docs: { price: "0.0", token: "MON" } },
      };
      NodeRouter.registerAgent("remote-node", mockManifest);
      const res = await NodeRouter.invokeAgent("remote-node", "search_docs", {
        query: "test",
      });
      expect(res.status).toBe("success");
      expect(res.output.agentId).toBe("remote-node");
    });
    ...
    it("should reject remote invocation if payment is required but missing", async () => {
      ...
      NodeRouter.registerAgent("remote-node", mockManifest);
      await expect(
        NodeRouter.invokeAgent("remote-node", "run_audit", {
          code: "contract X {}",
        }),
    ...
    it("should write registrations to the file system", () => {
      ...
      NodeRouter.registerAgent("temp-persisted-node", mockPeerManifest);
      ...
      expect(data["temp-persisted-node"]).toBeDefined();
      expect(data["temp-persisted-node"].name).toBe("Temp Node");

      NodeRouter.clearRegistry();
    });

    it("should reload registered nodes from file system on initialization", () => {
      NodeRouter.clearRegistry();
      ...
      const mockPeerManifest = {
        agentId: "offline-persisted-node",
      ...
      fs.writeFileSync(
        peersFilePath,
        JSON.stringify({
          "offline-persisted-node": mockPeerManifest,
        }),
        "utf-8",
      );

      const registered = NodeRouter.getRegisteredAgents();
      expect(registered["offline-persisted-node"]).toBeDefined();
      expect(registered["offline-persisted-node"].name).toBe(
  ...
  describe("NodeMarketplace", () => {
    it("should return pricing manifest, available skills and reputation", () => {
      const pricing = NodeMarketplace.getPricingManifest();
      expect(pricing.run_audit).toEqual({ price: "1.0", token: "MON" });

      const reputation = NodeMarketplace.getReputation();
      expect(reputation.score).toBe(98);

      const skills = NodeMarketplace.getAvailableSkills();
      expect(skills.length).toBeGreaterThan(0);
    });

    it("should record and filter execution history", () => {
      NodeMarketplace.recordExecution({
      ...
      const allHistory = NodeMarketplace.getExecutionHistory();
      expect(allHistory.length).toBe(1);
      expect(allHistory[0].skillName).toBe("search_docs");

      const filtered = NodeMarketplace.getExecutionHistory("non_existent");
      expect(filtered.length).toBe(0);

      const filteredSelf =
        NodeMarketplace.getExecutionHistory("monadforge-node");
  ...
  describe("EthersPaymentAdapter", () => {
    ...
    it("should query node from on-chain Registry contract fallback", async () => {
      ...
      const registeredBefore = NodeRouter.getRegisteredAgents();
      expect(registeredBefore["on-chain-node"]).toBeUndefined();
    ...
    it("should attach cryptographic work attestation to skill results", async () => {
      ...
      expect(result.attestation.agentId).toBe("monadforge-node");
    ...
    it("should handle error in on-chain Registry fallback gracefully", async () => {
      ...
      await expect(
        NodeRouter.invokeAgent("on-chain-node-fail", "search_docs", {
          query: "test",
        }),
    ...
    it("should compile NodeVerifier solidity contract successfully", async () => {
      const engine = new DeploymentEngine();
      const contractPath = path.resolve(process.cwd(), "contracts", "NodeVerifier.sol");
      const content = fs.readFileSync(contractPath, "utf-8");
      const compileRes = await engine.compile({
        "contracts/NodeVerifier.sol": content
      });
    ...
    it("should refresh peer cache from on-chain Registry contract fallback if TTL has expired", async () => {
      ...
      const mockPeerManifest = {
        agentId: "ttl-expired-node",
        name: "Expired Node",
        registeredAt: expiredTimestamp,
        pricing: { "search_docs": { price: "0.0", token: "MON" } }
      };
      NodeRouter.registerAgent("ttl-expired-node", mockPeerManifest);

      const getAgentSpy = jest.fn().mockResolvedValue([
        "Refreshed On-Chain Node",
        "",
        JSON.stringify({
          pricing: { "search_docs": { price: "0.0", token: "MON" } }
        })
      ...
      const res = await NodeRouter.invokeAgent("ttl-expired-node", "search_docs", { query: "test" });
      expect(res.status).toBe("success");
      expect(getAgentSpy).toHaveBeenCalledWith("ttl-expired-node");

      const registeredAfter = NodeRouter.getRegisteredAgents();
      expect(registeredAfter["ttl-expired-node"].name).toBe("Refreshed On-Chain Node");
      expect(registeredAfter["ttl-expired-node"].registeredAt).toBeGreaterThan(Date.now() - 5000);
