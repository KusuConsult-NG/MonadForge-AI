import { NodeRuntimeEngine } from "../src/index";
import * as fs from "fs";
import * as path from "path";

describe("NodeRuntimeEngine Unit Tests", () => {
  let runtime: NodeRuntimeEngine;
  const testProjectId = "node-project";

  beforeEach(() => {
    runtime = new NodeRuntimeEngine();
  ...
  it("should run a goal autonomously, compose skills, and save memory state", async () => {
    const result = await runtime.executeGoal(
      "Build a staking protocol on Monad for project node-project",
    );
  ...
  it("should trigger self-healing repair on compilation failures and succeed after retry", async () => {
    ...
    const mockPlan = await runtime["compositionEngine"].composeSkills(
      "Build a DEX protocol on Monad for project node-project",
    );
