...
describe("Agent HTTP Server & Client Tests", () => {
  ...
  beforeAll(async () => {
    ...
    server = new AgentServer(executor);
  ...
  it("should return the agent manifest via GET /manifest", async () => {
    const response = await fetch(`http://localhost:${testPort}/manifest`);
    expect(response.status).toBe(200);
    const data = await response.json();
    expect(data.agentId).toBe("monadforge-ai");
  });
  ...
  it("should route remote agent requests via real HTTP using AgentRouter", async () => {
    // Register remote agent with endpointUrl pointing to our test server
  ...
  it("should support start/stop lifecycles cleanly", async () => {
    const extraServer = new AgentServer();
  ...
  it("should sign payload using static helper", async () => {
    const sig = await AgentServer.signPayload("test-payload", privateKey);
    expect(sig.startsWith("0x")).toBe(true);
  });

  it("should reject start if port is already in use", async () => {
    const conflictServer = new AgentServer();
